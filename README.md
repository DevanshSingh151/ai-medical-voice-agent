...
**[🔗 View on GitHub](https://github.com/DevanshSingh151/ai-medical-voice-agent)**
...
VitalAgent follows a **layered architecture** with a service-oriented cut at the AI layer. Requests flow strictly downward — **Presentation → Authentication → Application/API → AI Services → Data** — and each layer only calls the layer directly beneath it:

| Layer | Responsibility | Technology |
|---|---|---|
| Presentation | Renders all user-facing screens | Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS v4, shadcn/ui |
| Authentication | Sign-in/up, sessions, route protection | Clerk Auth (JWT, OAuth) |
| Application / API | Request validation, business logic, AI orchestration | Next.js API Routes (`app/api/*`) |
| AI Services | Speech-to-text, voice orchestration, medical reasoning, text-to-speech | AssemblyAI, Vapi AI, Google Gemini / OpenAI, PlayHT |
| Data | Persistence | Neon Serverless PostgreSQL + Drizzle ORM |
| Containerization | Consistent runtime | Docker |
| Deployment | Hosting & delivery | Vercel |

A layered style (rather than full microservices) was chosen because the app runs as a single Next.js deployment on Vercel — this keeps each concern independently replaceable (e.g. swapping Gemini for another LLM only touches the AI-orchestration layer) without the operational overhead of separate services at this project's scale.

### Design Principles

- **Abstraction** — The 8 AI specialists are not hard-coded branches; each is described behind a common `DoctorAgent` shape (persona, system prompt, voice ID, tier), so the voice-call and report-generation code depends only on that interface, never on which specialist is active.
- **Modularity** — The codebase is split into independently testable modules that map onto the architecture: presentation (`app/`, `_components/`), authentication (`middleware.ts` + Clerk), API (one route file per capability), and data (`utils/schema.ts`).
- **High Cohesion** — Each module exposes functions that serve one purpose. `utils/subscription.ts`, for example, only answers plan-entitlement questions — it doesn't touch report schemas or the voice pipeline — so everything inside it changes for the same reason.
- **Low Coupling** — Layers communicate only through narrow REST/JSON interfaces and shared data shapes (`DoctorAgent`, `session_chart`), never shared internal state. The voice-call UI never calls Gemini directly; it hands the transcript to `/api/medical-report`, which owns that integration — so the AI provider can be swapped by changing one route.

### `DoctorAgent` — Specialist-Persona Abstraction

```ts
// data/doctorAgents.ts
export interface DoctorAgent {
  id: string;
  specialist: string;
  agentPrompt: string;   // system prompt for Vapi
  voiceId: string;       // cloned voice reference
  tier: "free" | "pro";
}

export const doctorAgents: DoctorAgent[] = [
  { id: "general-physician", specialist: "General Physician",
    agentPrompt: GENERAL_PHYSICIAN_PROMPT, voiceId: "voice_gp_01", tier: "free" },
  { id: "cardiologist", specialist: "Cardiologist",
    agentPrompt: CARDIOLOGIST_PROMPT, voiceId: "voice_cd_01", tier: "pro" },
  // ...6 more specialists, same shape
];
```

Adding a new specialist means appending one object to this array — the dashboard cards, the voice-call screen, and the paywall check all keep working unchanged because they only ever read the `DoctorAgent` interface.

### Figma Design

Full wireframe set (13 screens/states, desktop + mobile): **[VitalAgent — Figma Wireframes]
